...
*(DragonShoutSystem was previously on this list; it is now wired — see the Thu'um entry below.)*
...
- ✅ **Dragon Shouts wired (Thu'um)** — the previously deferred `DragonShoutSystem` is now a live combat verb: slaying a `DragonBoss` absorbs a Dragon Soul and echoes the next undiscovered Word of Power (shout catalogue order); souls unlock learned words in the new `ShoutUI` panel (**N** — Equip / Unlock-with-soul actions, word chips masked until discovered); **Shift+N** activates the equipped shout with per-reason failure feedback. `onShoutUsed` effects are delivered in `game.ts` (`_applyShoutEffects`): Unrelenting Force cone knockback + stagger (+ tier-3 damage), Whirlwind Sprint forward dash (raycast-clamped), Become Ethereal (100% `resist_damage` ActiveEffect), Fire Breath cone damage + burn DoT, Ice Form single-target freeze + frost damage, Slow Time (`_hostileTimeScale` squeezes the `updateNPCAI` clock while active), Elemental Fury (new `CombatSystem.externalAttackSpeedMultiplier` shortens melee/power-attack cooldowns), Clear Skies (`weatherSystem.forceWeather("clear")`). A persistent HUD chip shows the equipped shout, word pips, and READY / cooldown countdown; state persists via the existing `saveSystem.setDragonShoutSystem` hook.
...
- ✅ **MarkRecallSystem** — Morrowind's iconic Mysticism teleportation pair: `mark(position, cellId?)` stores a single magic waypoint anywhere in the world (including interior cells); `recall()` returns the stored position and cell id for the game layer to teleport the player — the mark is not consumed, so repeated recalls work just like in Morrowind; `clearMark()` removes the waypoint for scripted events; `onMark` / `onRecall` / `onMarkCleared` callbacks; `hasMarked` / `markedPosition` / `markedCellId` query accessors; defensive position copies prevent external mutation; safe round-trip save persistence (SAVE_VERSION 24).
- ✅ **TrainerSystem** — Morrowind/Oblivion NPC skill training: pay gold to a registered trainer to directly raise a skill level by 1 (up to the trainer's `maxLevel` cap); cost formula: `baseCost + currentLevel × costPerLevel`; enforces a `maxSessionsPerLevel` limit (default 5, mirrors Morrowind) that resets on `onCharacterLevelUp()`; non-mutating `canTrain()` eligibility check returns `TrainingCheckResult` with `canTrain` flag, optional denial `reason` (`unknown_trainer` / `skill_at_cap` / `session_limit_reached` / `insufficient_gold`), and cost; `train()` returns a discriminated `TrainingResult` and fires `onTrainingComplete(trainerId, skillId, newLevel, goldSpent)` for the game layer to update `SkillProgressionSystem` and deduct gold; nine built-in trainers cover Blade, Destruction, Sneak, Marksman, Speechcraft, Alchemy, and Restoration (both journeyman 50-cap and master 100-cap tiers); `registerTrainer()` / `removeTrainer()` / `getTrainersForSkill()` for data-driven extension; `sessionsThisLevel` / `sessionsRemaining` query accessors; save-state persistence (SAVE_VERSION 24).
...
- ✅ **WorldSeed — Minecraft-style seed generation** — `WorldSeed` (`src/world/world-seed.ts`, 39 tests) brings full Minecraft-style world randomisation: numeric or string seeds (strings are hashed via djb2 to a stable 32-bit integer); four `WorldType` options (`normal`, `flat`, `amplified`, `island`); four `BiomeScale` options (`small` / `medium` / `large` / `huge`) that control biome region size from tightly-varied to continent-scale; four `StructureDensity` options (`none` / `rare` / `normal` / `abundant`) that map to 0 / 10 / 25 / 45 % spawn thresholds; optional `startingBiome` pin that locks the 5×5 chunk area around the origin to a chosen biome; `flat` world type suppresses all structure spawning; `island` world type generates lush plains/forest near the origin and harsh tundra/desert on the periphery; `amplified` world type compresses biome values toward extremes for dramatic contrasts. `WorldManager` and `StructureManager` both accept an optional `WorldSeed` as a third constructor argument and fall back to their legacy deterministic formulas when none is supplied (zero breaking changes to existing worlds).
...
- ✅ **`NpcArchetypeSystem.spawnNpc()` expanded** — applies `aiProfile` overrides after role-based defaults; copies `voiceType`, `personalityTraits`, and `startingEquipmentIds` onto the spawned NPC entity; applies named schedule via `scheduleSystem.applySchedule()` when `scheduleId` is set.
...
- ✅ **`NpcScheduleSystem`** (`src/systems/npc-schedule-system.ts`, 42 tests) — Named schedule-definition registry that closes the gap between the `scheduleId` string on archetypes and the runtime `ScheduleBlock[]` list on NPC entities. `registerSchedule(def)` / `removeSchedule(id)` / `getSchedule(id)` / `registeredIds` for CRUD; `applySchedule(npc, id)` copies blocks onto `npc.scheduleBlocks` and maps per-block `anchorPosition` values to `npc.workPosition` (for `"work"` blocks) and `npc.homePosition` (for `"sleep"` blocks) when those fields are not already set. Four built-in presets registered automatically: `sched_guard` (patrol 06–22, sleep 22–06), `sched_villager` (work 08–18, wander 18–22, sleep 22–08), `sched_innkeeper` (work 08–23, sleep 23–08), `sched_merchant` (work 08–20, wander 20–22, sleep 22–08). Attach to `NpcArchetypeSystem.scheduleSystem` to apply schedules automatically on every `spawnNpc()` call.
- ✅ **Test coverage** — `npc-creator-system.test.ts` expanded to 50 tests; `npc-archetype-system.test.ts` expanded to 23 tests; `npc-schedule-system.test.ts` 42 tests.
...
### Architecture Alignment — Headless Core / Babylon Adapter / Electron Shell ✅

Formalises the three-layer boundary (headless core, Babylon adapter, Electron shell) that was already implicit in the codebase. Adds the missing adapter layer, offscreen NPC simulation, and DevConsole plan:

1. ✅ **Adapter layer directory** (`src/adapters/babylon/`, barrel `index.ts`) — explicit boundary between headless systems and Babylon-specific code.
2. ✅ **BabylonInputAdapter** (`src/adapters/babylon/babylon-input-adapter.ts`, 25 tests) — Decouples input action mapping from the game loop. Maps keyboard/mouse events to named `InputAction`s (meleeAttack, toggleInventory, pause, etc.). Supports headless simulation (`simulateAction`/`simulatePress`), runtime rebinding, modifier-key guards (shift, ctrlOrMeta), case-insensitive matching, phase-aware dispatch (down/up), active-action tracking.
3. ✅ **BabylonCharacterControllerAdapter** (`src/adapters/babylon/babylon-controller-adapter.ts`, 31 tests) — Pure-TypeScript first-person movement state. Tracks position, velocity, crouch, sprint, swim, mount, jump, gravity. Yaw-rotated input with diagonal-normalisation. Snapshot/restore for save compatibility. Callbacks: onCrouchToggle, onJump, onLand.
4. ✅ **BabylonSceneHost** (`src/adapters/babylon/babylon-scene-host.ts`, 20 tests) — Scene-level visual configuration (lighting, sky, fog, shadows, post-processing) stored as pure data. Validation for shadow map power-of-two, positive exposure/contrast, non-negative fog density. Snapshot/restore. Mutation helpers for weather and time-of-day integration.
5. ✅ **OffscreenSimulationSystem** (`src/systems/offscreen-simulation-system.ts`, 32 tests) — Headless core system for simulating NPC state while unloaded. Advances schedule behavior, regenerates health (5 HP/game-hour), regenerates merchant gold (2 gold/game-hour). Provides reconciled `OffscreenNpcState` (behavior, expected position, health, isSleeping) when NPCs return to view. Snapshot/restore for save persistence.
6. ✅ **DevConsole planned** — `docs/electron-plan.md` updated with DevConsole module spec: live system inspection, command execution, log streaming, autocomplete, security sandboxing.
7. ✅ **Architecture docs updated** — `docs/framework-architecture.md` documents the adapter layer and offscreen simulation system.

Total: 108 new tests across 4 test files.
