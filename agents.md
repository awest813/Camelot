...
- **UI**: `src/ui/character-creation-ui.ts` — `CharacterCreationUI.open()` returns a `CharacterCreationResult` (`name`, `raceId`, `birthsignId`, `classId`, `skipGameplayTips`, `worldSeed`).
- **Flow**: Welcome (copy + optional “skip gameplay tips”) → World (seed / generation settings) → Name → Race → Birthsign → Class → resolve.
